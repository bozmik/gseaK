...
plot(ES_matrix[,3],lwd=3,type = "l",col='blue',ylab =c('Cumulative distribution'),xlab = '')
lines(ES_matrix[,4],col='red',lwd=3)
...
plot(1:N,as.vector(ES_matrix[,5]),axes=F,pch=20,ylab='Enrichment Score',xlab=NA,ylim =c(min(ES_matrix[,5])-0.1,1),type="l",col='blue',lwd = 3,main=name_gene_set)
lines(c(1:N),c(rep(0,N)),lwd=1,lty=1,col="black")
lines(c(x_ES,x_ES),c(0,ES_obs),col='red',lty=10,lwd=2)
text(x_ES,ES_obs+0.1, round(ES_obs, 4),cex=1)
Axis(side=2,las = 1, tck = -0.02)
par(mar=c(5.5,4.5,2, 0.5))
plot(set,rep(0.1,length(set)),axes=F,type='h',col='blue',ylim=c(0,0.1),xlim=c(1,N),ylab =NA,xlab=NA)
